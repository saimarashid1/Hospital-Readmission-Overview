...
WITH readmission_stats AS (
  SELECT 
    p.pat_age,
    COUNT(*) AS total_patients,  -- Or COUNT(DISTINCT e.pat_id) for unique patients
    SUM(CASE WHEN e.readmitted = 'yes' THEN 1 ELSE 0 END) AS readmitted_patients
  FROM patients p
  INNER JOIN Encounters e ON p.pat_id = e.pat_id
  WHERE e.readmitted IS NOT NULL  -- Optional: Exclude unknowns
  GROUP BY p.pat_age  -- Group only by age for overall rates
)
SELECT 
  pat_age,
  total_patients,
  readmitted_patients,
 FORMAT((readmitted_patients * 1.0 / total_patients), 'P2') AS readmission_rate_pct
FROM readmission_stats
ORDER BY pat_age;--- Optional: Sort by age
...
--Diabetes Medication Changes and Readmission Correlation

select count(*) as total_patients , d.diabetes_med as diabetic_patients,d.change as changed_med,
sum ( case when e.readmitted  = 'yes' THEN 1 ELSE 0 END) as readmitted_diagpatients,
Format (1.0 *sum ( case when e.readmitted  = 'yes' THEN 1 ELSE 0 END)/count(*) ,'p2')
		as Readmission_Rate_Percent
from Diagnosis d
...
select medical_specialty, CAST(sum(CAST(n_inpatient AS INT)) AS INT) AS total_inpatients,
